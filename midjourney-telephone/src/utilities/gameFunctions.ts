import { initializeGame, saveImage, updateGameWithGuess } from "./firebase/firebaseWriteFunctions";
import { Game, GameType, Guess, GuessAndId } from "./types";

const getBase64FromUrl = async (url: string): Promise<string | null | ArrayBuffer> => {
...
export async function generateImage(prompt: string, seed: number = 2) {
	const data = {
	...
	headers.append("Authorization", `Bearer ${process.env.NEXT_PUBLIC_IMG_API_KEY}`);
...
export async function createNewGame(
	...
	invitedUsers?: string[],
	description?: string,
	allowReplay?: boolean): Promise<Guess | null> {
	...
	if (res) {
		...
		const initializationRes = await initializeGame(newGame, gameId, initialGuess, guessId);
...
export async function submitGuess(
	gameId: string, userId: string, imageString: string, description: string, prevGuess: GuessAndId): Promise<boolean> {
		const imageFilename = firestoreAutoId() + ".jpeg";
		const res = await saveImage(imageString, imageFilename);
		...
		if (res) {
			...
			const status = await updateGameWithGuess(gameId, guess, guessId, prevGuess.id, userId);
