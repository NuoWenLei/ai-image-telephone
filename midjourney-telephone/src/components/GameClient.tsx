...
import { getImageURL, getUserGuess } from "@/utilities/firebase/firebaseReadFunctions";
import { AuthContext } from '@/utilities/firebase/firebaseAuthProvider';
import { useRouter } from 'next/navigation';
import { DESCRIPTION_MAX_LEN, DESCRIPTION_MIN_LEN } from '@/utilities/constants';
...
export default function GameClient({ game, prevGuess } : GameClientProp) {
	...
	if (authState.user == null) {
		...
		(async () => {
			...
			if (imageUrl){
				...
				toast.error("Error loading previous image, please refresh and try again");
			}
		})()
	}
  ...
  async function onGenerate() {
	...
	const validDescription = (inputDescription.length >= DESCRIPTION_MIN_LEN) && (inputDescription.length <= DESCRIPTION_MAX_LEN)

	if (!validDescription) {
		toast.error(`Input length must be <${DESCRIPTION_MIN_LEN} and >${DESCRIPTION_MAX_LEN}`);
		setLoading(false);
		return;
	}

    const base64Image = await generateImage(inputState, prevGuess.guess.seed);
  ...
  return (
    <>
    {loading && <div className="fixed inset-0 bg-black/30" aria-hidden="true" />}
    <main className="flex h-full w-full flex-col items-center justify-between pb-4">
      { img ?
	  <img className="h-96 w-96 xl:h-[30rem] xl:w-[30rem]" src={img} alt="generated image" /> :
	  <div className="h-96 w-96 text-center flex flex-col justify-center"><p>Loading...</p></div> }
	  {
		...
		<>
			<div className="flex flex-col w-1/2 bg-black ring-[6px] ring-white rounded-lg outline-none p-2 mt-3">
				<textarea
					...
					className={"w-full h-[8vh] text-sm bg-black bg-transparent outline-none resize-none p-2 duration-150 text-white "}
			...
			<button
			disabled={loading}
			className="py-2 px-4 text-black bg-white rounded-md mt-3 outline-none focus-none" onClick={onGenerate}>
				Generate
			</button>
		</> :
