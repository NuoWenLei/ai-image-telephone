...
import { DESCRIPTION_MAX_LEN, DESCRIPTION_MIN_LEN, GAME_NAME_MAX_LEN } from "@/utilities/constants";
...
export default function GameCreateClient({userEmail} : { userEmail: string | null }) {
	...
	function updateGameName(s: string) {
		setGameName(s.trim().replace(/[^a-zA-Z0-9]/g, '').substring(0, GAME_NAME_MAX_LEN));
	}
	...
	async function onSubmit() {
		...
		const result = profanity(trimmedGameName);
		if ((result != false) && (result.badWordLanguage == "en")) {
			toast.error(`Game name cannot contain ${result.detectedWord}`);
			setLoading(false);
			return;
		}

		if ((trimmedGameName.length == 0) || (trimmedGameName.length > GAME_NAME_MAX_LEN)) {
			toast.error(`Game name must be between 1 to ${GAME_NAME_MAX_LEN} characters long`);
			setLoading(false);
			return;
		}
		...
		const invalidDescription = ((trimmedInitialDescription.length > DESCRIPTION_MAX_LEN) || (trimmedInitialDescription.length < DESCRIPTION_MIN_LEN));
	...
	<main className="flex h-[90vh] w-screen flex-col justify-center pb-4 text-white font-bold text-4xl">
		...
		<div className="w-[40%] h-[70vh] flex flex-col gap-8 mx-auto px-4 py-2 overflow-y-auto">
			<div>
				...
				className={"w-full h-[12vh] rounded-lg text-4xl bg-black rounded-lg ring-[8px] ring-white outline-none px-4"}/>
			  </div>
			  <div>
				<div className="w-full h-[8vh] flex flex-row gap-6">
					...
					className={"w-1/2 h-full " + classes + (gameType == "Public" ? " bg-white text-black " : "")}>
						<span className="mx-auto">PUBLIC</span>
					</button>
					<button type="button"
					onClick={() => setGameType("Private")}
					disabled={loading}
					className={"w-1/2 h-full " + classes + (gameType == "Private" ? " bg-white text-black " : "")}>
			...
			<div className={(gameType == "Public" ? " hidden " : "")}>
				...
				className={"w-full h-[24vh] text-sm bg-black ring-[6px] ring-white rounded-lg outline-none resize-none p-2 duration-150 "
				+ (gameType == "Public" ? " hidden " : "")}
				placeholder="Emails to invite (separate by commas)">
				</textarea>
			 </div>
			 <div>
				<div className="w-full h-[6vh] flex flex-row cursor-pointer gap-6" onClick={() => (loading ? null : setAllowReplay(!allowReplay))}>
					<div className={`w-[7vh] h-full text-4xl mx-auto outline-none rounded-lg ring-[8px] ring-white 
						text-center flex flex-col justify-center `}>
						{allowReplay && <CheckIcon fontSize="inherit" className="mx-auto"/>}
					</div>
					<div className={"outline-none w-full h-full text-center flex flex-col justify-center select-none"}>
						ALLOW REPLAY
					</div>
				</div>
			</div>
			<div>
				<div className="flex flex-col w-full bg-black ring-[6px] ring-white rounded-lg outline-none p-2 ">
					<textarea
						...
						className={"w-full h-[24vh] text-sm bg-black bg-transparent outline-none resize-none p-2 duration-150 "}
			...
			<div>
				...
				disabled={loading}
				onClick={onSubmit}
				className={"w-full h-[8vh] bg-white text-black rounded-lg ring-[6px] ring-white outline-none"}>
					CREATE
				</button>
			</div>
		</div>
